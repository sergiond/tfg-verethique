            ...
            <div className="mx-auto max-w-5xl px-6 md:px-8 py-20">
                <div className="flex flex-col lg:flex-row gap-16 mb-24">
                    <div className="lg:w-2/3">
                        <p className="font-serif text-2xl md:text-3xl text-[#1A1A1A] leading-snug mb-8">Resumen del Dossier.</p>
                        <p className="font-sans text-lg text-[#1A1A1A]/70 leading-relaxed mb-10">{brand.description}</p>
                    </div>

                    <div className="lg:w-1/3 flex flex-col gap-4">
                        {brand.web ? (
                            <a href={brand.web} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 w-full px-6 py-4 bg-[#1A1A1A] text-white rounded-full font-mono text-xs uppercase tracking-widest hover:bg-[#CC5833] transition-colors">
                ...
                <div className="mb-24">
                    ...
                    <div className="bg-white border border-[#D1CFC7] rounded-[2rem] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                        <div className="p-10 md:p-12 border-b border-[#D1CFC7] bg-[#ECEAE3]">
                            ...
                            <p className="font-sans text-[#1A1A1A]/70 mb-10 max-w-2xl leading-relaxed">Hemos corroborado las afirmaciones de esta marca cruzando datos públicos con los indicadores ASG seleccionados en revisión.</p>
                                ...
                                <ul className="grid sm:grid-cols-2 gap-6">
                                    {selectedIndicators.map((indicator) => (
                                        <li key={indicator.id} className="flex items-start gap-4 bg-white p-6 rounded-[1.5rem] border border-[#D1CFC7]/50 shadow-sm">
                                            ...
                                            <div>
                                                <p className="font-mono text-[9px] uppercase tracking-widest text-[#CC5833] mb-2">{dimensionLabels[indicator.dimension]}</p>
                                                <p className="font-serif text-xl text-[#1A1A1A] mb-1">{indicator.title}</p>
                                                <p className="font-sans text-sm text-[#1A1A1A]/60 leading-relaxed">{indicator.description}</p>
                                            </div>
                                        </li>
                                    ))}
                                </ul>
                            ) : (
                                <p className="rounded-[1.5rem] border border-[#D1CFC7]/50 bg-white p-6 font-sans text-sm text-[#1A1A1A]/60">
                                    No hay indicadores ASG estructurados marcados para esta marca.
                                </p>
                            )}
                        </div>
                        ...
                        <div className="p-10 md:p-12">
                            ...
                            <p className="font-sans text-[#1A1A1A]/70 mb-8 max-w-2xl leading-relaxed">Indicadores no marcados todavía en la revisión documental. Su ausencia reduce la puntuación de la dimensión correspondiente.</p>
                                ...
                                <ul className="grid gap-4 sm:grid-cols-2">
                                    {missingIndicators.map((indicator) => (
                                        <li key={indicator.id} className="flex items-start gap-4 rounded-[1.25rem] border border-[#D1CFC7]/50 bg-[#F2F0E9]/50 p-5">
                                            <span className="text-[#CC5833] mt-1 font-mono">↳</span>
                                            <div>
                                                <p className="font-mono text-[9px] uppercase tracking-widest text-[#1A1A1A]/40 mb-1">{dimensionLabels[indicator.dimension]}</p>
                                                <p className="font-serif text-lg text-[#1A1A1A]">{indicator.title}</p>
                                                <p className="font-sans text-sm text-[#1A1A1A]/60 leading-relaxed mt-1">{indicator.evidenceHint}</p>
                                            </div>
                                        </li>
                                    ))}
                                </ul>
                            ) : (
                                <p className="font-sans text-sm text-[#1A1A1A]/70">No quedan brechas estructuradas pendientes en esta metodología.</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-[#1A1A1A] p-8 md:p-12 rounded-[2rem] flex flex-col md:flex-row gap-6 md:gap-8 items-start relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="font-mono text-[10px] text-white/40 uppercase tracking-[0.2em]">Última Revisión de Inteligencia: Octubre 2023</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white p-10 flex flex-col">
            <div className="flex justify-between items-start mb-12">
                <h3 className="font-serif text-3xl text-[#1A1A1A]">
                    {title} <span className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1A1A]/40 mt-2 font-normal">{subtitle}</span>
                </h3>
